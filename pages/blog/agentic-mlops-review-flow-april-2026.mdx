...
In this scenario, the current champion has an RMSE of 150,000 and the candidate comes in at around 155,000. It performs worse, but not devastatingly worse. It is a ~3% regression, which is still below the hard overall regression threshold. Drift is also clean in that run, so if I only looked at overall metrics and drift, the candidate might look like a borderline but manageable case.

The problem shows up when the workflow compares segment performance. For the town segment `YISHUN`, the candidate RMSE jumps from 150,000 for the champion to 195,000 for the candidate. That is a 30% regression for that subgroup. In my policy, segment regressions above 20% do not auto-reject the model, but they do force `MANUAL_REVIEW`.
...
I like this example because it shows why I did not want the agentic layer to own the promotion decision. The agent can help explain why Yishun got worse and whether there is any surrounding market context worth inspecting, but it should not get to decide that a 30% subgroup regression is acceptable. That is a policy question and, after policy has spoken, a human accountability question.
...
The second is segmented regression. It may reduce the overall error a little, but it can make the results much worse for one type of flat or one town. That is exactly the kind of failure that an average metric can hide and a business team will care about. I push those segmented metrics through the entire workflow so the policy engine can force manual review when a subgroup regresses. The agent can then help explain why the regression matters and whether there is any surrounding context worth considering.
...
Invest in observability earlier than feels necessary. Teams often want better prompts, more tools, or a stronger model. Those can help, but they do not solve the trust problem. Persist the review segments. Record the tool outputs. Tag the final decision. Keep model version history clean. Test scenario fixtures that represent real promotion states.
...
One thing worth calling out: the public repo does not contain a full MLOps pipeline. What you will find are ad-hoc notebook-based jobs that walk through the promotion review flow. The actual Sagemaker pipeline DAG, including scheduling and orchestration, lives in the MAESTRO internal docs and is not exposed publicly. That said, the notebooks are structured so they could be run as scheduled CRON jobs to drive real deployments. I kept the repo scoped this way deliberately. The goal was learning and demonstration, not shipping a production-grade pipeline template. The ideas transfer; the operational wiring is something each team will need to fit to their own platform.
